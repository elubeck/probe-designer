    ...
    def get_intron(self, row, chrom_seq):
        ...
        for coord_pair in coords_str.split(", ("):
            ...
            for sub_seq in seq.seq.split("N"):
                if len(sub_seq) >= 20:
                    record = SeqRecord(sub_seq, name=name2, id=name2, description='')
                    yield record

    def __iter__(self):
        for chromosome in self.table.distinct('chrom'):
            chrom_code = chromosome['chrom']
            chrom_path = self.chromo_folder.joinpath("{}.fa.masked".format(chrom_code))
    ...
    for n, gene in enumerate(IntronRetriever()):
        ...
        if len(chunks) == chunksize:
            with NamedTemporaryFile("w") as fasta_input:
                for gene in chunks:
                    SeqIO.write(gene, fasta_input, 'fasta') # write all sequences to file
                fasta_input.flush()
