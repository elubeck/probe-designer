...
f_probes = {k: ['TATA' + seq for seq in p_set2[k]]
...
primers = [[ 'ACAGTGGAACGCGCCATGAG','CGCTTGTTGGCTGGTATGCG' ]
            ...
            ,[ 'CATGCCAGATCCCAATCCCC','CAATCTGCGGGCATGTTTCG' ]
            ,[ 'TCCATGAGCTCTGTTGCGGG','CCGGCCTGAAAAACGTAGCG' ]]

import get_seq    
p2 = [(forward, get_seq.reverse_complement(reverse)) for forward, reverse in primers]
...
for n, start in enumerate(range(0, len(intron_probes2), 100)):
    ...
    for gene in intron_probes2[start:start+100]:
        for probe in gene:
            ...
            Spacer2 = 'GAT'
            EcoRI = 'GAATTC'
            schematic = " ".join([fp, ScaI , Spacer1 , probe['seq'] , Spacer2 , EcoRI , rp])
